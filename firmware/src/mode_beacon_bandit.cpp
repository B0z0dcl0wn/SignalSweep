...
static TaskHandle_t spoofTaskHandle = NULL;
static String currentSpoofPayload = "";

static void spoofTask(void* pvParameters) {
    ESP_LOGI(TAG, "Starting 10-second BLE Spoofing Blast...");
    ...
    size_t len = currentSpoofPayload.length() / 2;
    uint8_t* payloadBytes = new uint8_t[len];
    for (size_t i = 0; i < len; i++) {
        String byteStr = currentSpoofPayload.substring(i * 2, i * 2 + 2);
        payloadBytes[i] = (uint8_t) strtol(byteStr.c_str(), NULL, 16);
    }
    
    // Add flags if not present in payload
    advert.addData(std::string((char*)payloadBytes, len));
    pAdvertising->setAdvertisementData(advert);
    
    // Removed spoofing mac randomization for stability
    pAdvertising->start();
    
    // Blast for exactly 10 seconds
    vTaskDelay(pdMS_TO_TICKS(10000));
    
    pAdvertising->stop();
    delete[] payloadBytes;
    
    ESP_LOGI(TAG, "Spoofing Blast Complete. Restoring BLE Serial Advertising...");
    restoreBleSerialAdvertising();
    
    if (banditRunning) {
        pScan->start(0, nullptr, false);
    }
    
    spoofTaskHandle = NULL;
    vTaskDelete(NULL);
}

void executeBleSpoof(const String& hexPayload) {
    if (spoofTaskHandle != NULL) {
        ESP_LOGW(TAG, "Spoofing already in progress. Ignoring request.");
        return;
    }
    
    currentSpoofPayload = hexPayload;
    xTaskCreatePinnedToCore(
        spoofTask,
        "SpoofTask",
        4096,
        NULL,
        2, // Higher priority than scan
        &spoofTaskHandle,
        1
    );
}
